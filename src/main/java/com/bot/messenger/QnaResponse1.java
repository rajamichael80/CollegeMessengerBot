...
public class QnaResponse1 {
	...
	public String getJsonResponse(String senderId, String requestText, UserDetail userDetail) {
		String jsonResponse = "";
		String key = getActualKeyword(requestText);
		/*
		 * switch (getActualKeyword(requestText)) { case "hi": jsonResponse =
		 * String.format(
		 ...
		 * ); break; case "course": jsonResponse =
		 * "{ \"recipient\":{ \"id\":\"recipientId\" }, \"message\":{ \"attachment\":{ \"type\":\"template\", \"payload\":{ \"template_type\":\"generic\", \"elements\":[ { \"title\":\"Courses\", \"image_url\":\"https://college-messanger-bot.herokuapp.com/img/courses_offered.gif\", \"subtitle\":\"Here is cousre details\", \"buttons\":[ { \"type\":\"postback\",  \"title\":\"MCA\" ,\"payload\":\"MCADemo\"}, { \"type\":\"web_url\", \"url\":\"http://www.newprinceshribhavani.com/computer-science.php\", \"title\":\"Information Technology\" }, { \"type\":\"postback\", \"title\":\"Any Other Query\", \"payload\":\"call our representative\" } ] }  ] } } } }"
		 * ; break; case "time": jsonResponse = String.
		 * format("{ \"recipient\": { \"id\": \"recipientId\" }, \"message\": { \"text\": \"%s\" } }"
		 * ,"College start at 9.00 AM and ends at 4.30PM"); break;
		 * 
		 * case "call our representative": jsonResponse = String.format(
		 * "{ \"recipient\": { \"id\": \"recipientId\" }, \"message\": { \"text\": \"%s\" } }"
		 * , "Hi " + userDetail.getFirstName() + " " + userDetail.getLastName()
		 ...
		 * ); break; case "bye": case "": jsonResponse = String.format(
		 * "{ \"recipient\": { \"id\": \"recipientId\" }, \"message\": { \"text\": \"%s\" } }"
		 * , "Grazie " + userDetail.getFirstName() + " " + userDetail.getLastName()
		 * +", Speriamo di aver risposto alle tue domamde.  Buona giornata "); break;
		 * 
		 * default:
		 * 
		 * jsonResponse =
		 * "{ \"recipient\":{ \"id\":\"recipientId\" }, \"message\":{ \"attachment\":{ \"type\":\"template\", \"payload\":{ \"template_type\":\"button\", \"text\":\"I could not understand the request, if you wish I can put you in touch with one of our assistants.\", \"buttons\":[ { \"type\":\"phone_number\", \"title\":\"Contact Number\", \"payload\":\"+390152434600\" } ] } } } }"
		 * ; break; }
		 */
		JSONConversionUtil conversionUtil = new JSONConversionUtil();

		Map<String, String> qnaResponse = conversionUtil.convertXmlToObject().getQnaResponse();
		System.out.println("----qnaResponse----" + qnaResponse);
		jsonResponse = qnaResponse.get(key);
